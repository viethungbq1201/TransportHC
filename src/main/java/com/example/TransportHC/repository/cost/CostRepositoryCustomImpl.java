...
public class CostRepositoryCustomImpl implements CostRepositoryCustom {
        ...
        @Override
        public List<Cost> findCostsByTruck(Long truckId, LocalDate from, LocalDate to) {
                return queryFactory
                                .selectFrom(cost)
        ...
        @Override
        public BigDecimal sumCostByTruck(Long truckId, LocalDate from, LocalDate to) {
                return queryFactory
        ...
        public BigDecimal sumAllCosts(LocalDate from, LocalDate to) {
                return queryFactory
                                .select(cost.price.sum().coalesce(BigDecimal.ZERO))
                                .from(cost)
                                .where(cost.date.between(from, to),
                                                cost.approveStatus.eq(ApproveStatus.valueOf("APPROVED")))
                                .fetchOne();
        }

        @Override
        public Map<Long, BigDecimal> sumCostBySchedule(List<Long> scheduleIds) {
        ...
        @Override
        public BigDecimal sumCostByDriver(Long driverId, LocalDate from, LocalDate to) {
                return queryFactory
                                .select(cost.price.sum().coalesce(BigDecimal.ZERO))
                                .from(cost)
                                .where(
